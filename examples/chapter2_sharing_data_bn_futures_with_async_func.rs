...
async fn main() {
    ...
    let handle_one: JoinHandle<u32> =
        tokio::task::spawn(count(3, shared_data, CounterType::Increment));

    let handle_two: JoinHandle<u32> =
        tokio::task::spawn(count(3, shared_two, CounterType::Decrement));

    (_, _) = tokio::join!(handle_one, handle_two);
}
